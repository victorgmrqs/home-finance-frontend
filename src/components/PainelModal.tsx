...
export const PainelModal = ({ isOpen, onClose, painel }: PainelModalProps) => {
  const [formData, setFormData] = useState({ nome: "", descricao: "", usuario_id: "" });
  ...
  useEffect(() => {
    if (painel) {
      setFormData({ nome: painel.nome, descricao: painel.descricao || "", usuario_id: painel.usuario_id.toString() });
    } else {
      setFormData({ nome: "", descricao: "", usuario_id: "" });
    }
    setErrors({});
  }, [painel, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    const data: any = { nome: formData.nome.trim(), usuario_id: parseInt(formData.usuario_id) };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[400px]">
        <DialogHeader><DialogTitle>{isEditMode ? "Editar Painel" : "Novo Painel"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div><Label htmlFor="nome">Nome <span className="text-destructive">*</span></Label><Input id="nome" value={formData.nome} onChange={(e) => handleChange("nome", e.target.value)} placeholder="Ex: Casa, Pessoal" className={errors.nome ? "border-destructive" : ""} />{errors.nome && <p className="text-sm text-destructive mt-1">{errors.nome}</p>}</div>
          <div><Label htmlFor="descricao">Descrição</Label><Input id="descricao" value={formData.descricao} onChange={(e) => handleChange("descricao", e.target.value)} placeholder="Ex: Gastos da casa" /></div>
